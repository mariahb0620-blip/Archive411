  ...
  useEffect(() => {
    async function load() {
      ...
      if (session && session.lookbook.id === id) {
        ...
        setCatalogProducts(session.products ?? productsForLooks(session.looks));
      ...
      if (persisted) {
        ...
        setCatalogProducts(persisted.products ?? productsForLooks(persisted.looks));
      ...
      <AppPageMain stickyFooter={!saved && !isEmpty}>
        ...
        <header className="border-b border-smoke/30 pb-6 md:pb-8">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-[10px] uppercase tracking-[0.2em] text-smoke">
            {lookbook.occasion && (
              <span className="rounded-full border border-smoke/40 px-2 py-1">{lookbook.occasion}</span>
            )}
            {lookbook.location && (
              <span className="rounded-full border border-smoke/40 px-2 py-1">{lookbook.location}</span>
            )}
            {lookbook.climate && (
              <span className="rounded-full border border-smoke/40 px-2 py-1">{lookbook.climate}</span>
            )}
          </div>
          <div className="mt-6 hidden md:block">
            ...
            {saved && (
              <Link
                href="/archive"
                className="ml-4 text-xs uppercase tracking-[0.2em] text-accent hover:underline"
              >
                View Archive
              </Link>
            )}
